...
#=========================================================================================
#SBATCH --job-name=MLP-Overparam-Set-I
...
declare -a d_values=(2048 4096 8192 16384 32768) # Adjust as you like.
declare -a activation_values=('sin' 'relu')
declare -a seed_values=(1337 1338 1339)

# EXPLICIT train_size sweep (edit these integers directly)
# Example values for p=97 (p^2=9409). Adjust as you like.
declare -a train_size_values=(4000 8000)

# Weight decay sweep
declare -a weight_decay_values=(0.3 0.1 0.03 0.01 0.003 0.001 0) # Adjust as needed.
...
# (train_size) -> seed -> activation -> weight_decay -> d -> (m,p)
...
# --- Run Python Script ---
python /home-nfs/leo/project/Grokking/mlp.py \
  --project "Overparam-ICLR" \
  --m "$m" \
  --p "$p" \
  --d "$d" \
